...
const SEATS_PER_PEW = 5;

const seatSpacing = layout.seatSpacing;
const rowSpacing = layout.rowSpacing;
const rowOffset = layout.rowOffset || 0;
const aisleHalfWidth = layout.aisleHalfWidth;
const aisleGap = layout.aisleGap;

// All rows: 2 pews, 5 seats each
const sideCenterOffset = aisleHalfWidth + aisleGap + ((SEATS_PER_PEW - 1) * seatSpacing) / 2;

export function buildPews(scene, totalRows) {
    const pewDepth = 0.6;
    const pewHeight = 0.55;
    const pewWidth = SEATS_PER_PEW * seatSpacing;
    ...
    for (let row = 0; row <= totalRows; row += 1) {
        ...
        // All rows: 2 pews (left and right of aisle)
